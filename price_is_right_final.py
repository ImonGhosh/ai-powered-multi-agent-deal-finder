...
class App:
    ...
    def run(self):
        with gr.Blocks(title="The Price is Right", fill_width=True) as ui:
            ...
            # Yields updated log data, html formatted logs, and final result when available, else initial result (i.e current memory)
            ...
            def run_with_logging(initial_log_data):
                ...
                # Starts the agent framework worker in a separate thread, so that UI table update and log/deal fetching can happen concurrently
            ...
            with gr.Row():
                gr.Markdown('<div style="text-align: center;font-size:24px"><strong>AI Powered Live Deal Finder</strong> - An autonomous Multi Agent Framework that hunts the best deals for you</div>')
            with gr.Row():
                gr.Markdown('<div style="text-align: center;font-size:14px">Our proprietary fine-tuned LLM and a RAG pipeline with a frontier model collaborate to render a live stream of great online deals with push notification functionality.</div>')
            with gr.Row():
            ...
            # Load initial log and opportunities
            ui.load(run_with_logging, inputs=[log_data], outputs=[log_data, logs, opportunities_dataframe])

            # Every 5 minutes, run again
            # Returns log_data (list of logs), logs (html formatted logs) and opportunities_dataframe (deals)
